...
_gs_type = _gs_secrets.get("type")
if _gs_type != "service_account":
    st.error(
    ...
    st.caption(f"Valeur de 'type' actuellement detectee : {_gs_type!r}")
    st.stop()
...
with st.sidebar:
    ...
    with st.expander("🔧 Diagnostic connexion Google Sheets"):
        st.write("Type detecte :", repr(_gs_type))
        st.write("Spreadsheet configure :", repr(_gs_secrets.get("spreadsheet")))
        st.write("Client email :", repr(_gs_secrets.get("client_email")))
        st.caption(
            "Si le type ci-dessus est bien 'service_account' mais que l'erreur "
            "UnsupportedOperationError persiste, la connexion a probablement ete mise "
            "en cache avec une ancienne configuration. Clique ci-dessous pour forcer "
            "une reconnexion complete."
        )
        if st.button("🔄 Forcer la reconnexion (vider le cache)"):
    ...
    if measurements.empty:
        ...
        with st.expander("Historique complet"):
            st.dataframe(measurements_sorted.sort_values("date", ascending=False),
                         use_container_width=True, hide_index=True)# -*- coding: utf-8 -*-
"""
Dashboard de suivi nutrition & performance - sportif (musculation + basket)
Suivi quotidien / hebdomadaire des apports par categorie + poids & mensurations.
Lancer avec : streamlit run app.py
